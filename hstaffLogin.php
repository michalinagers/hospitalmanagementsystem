...
function verifyUsersStaff($hstaffemail, $hstaffpassword) {
    $db = new SQLite3("F:\\xampp\\htdocs\\hospital_database.db");
    $stmt = $db->prepare('SELECT * FROM hospitalstaff WHERE hstaff_email=:hstaffemail AND password=:hstaffpassword');
    // Bind parameters
    $stmt->bindParam(':hstaffemail', $hstaffemail, SQLITE3_TEXT);
    $stmt->bindParam(':hstaffpassword', $hstaffpassword, SQLITE3_TEXT);
...
if (isset($_POST['hstafflogin'])) {
    $hstaffemail = $_POST['hstaffemail'];
    $hstaffpassword = $_POST['hstaffpassword'];

    $userStaffData = verifyUsersStaff($hstaffemail, $hstaffpassword);
    ...
    if (count($userStaffData) == 0) {
        ...
        $_SESSION['hstaffname'] = $userStaffData[0]['hstaff_fname'];
        $_SESSION['hstaff_id'] = $userStaffData[0]['hstaff_id'];
        header("Location: hStaffMainpage.php"); //redirect to the dashboard page
    }
}
...
<body>
    ...
    <p class="title">Login as Staff</p>
    ...
    if (<?php echo $invalidMesg !== "" ? "true" : "false"; ?>) {
        ...
        <input type="email" id="email" name="hstaffemail" required><br>
        <label for="pwd">Password <span class="required">*</span></label><br>
        <input type="password" id="pwd" name="hstaffpassword" required><br>
        <input type="submit" value="Login" name="hstafflogin">
      </form>
    </div>
